...
/* START BUTTON CLICK - placeholder for future use */
...
photos.forEach(photo=>{
  photo.addEventListener("click", ()=>{
    ...
    overlay.innerHTML=`<button id="overlay-close">✕</button><img src="${src}">`;
    overlay.style.display="flex";
    requestAnimationFrame(()=>{ overlay.style.opacity="1"; overlay.style.pointerEvents="auto"; });
    document.getElementById("overlay-close").addEventListener("click", closeOverlay);
  });
});
function closeOverlay(){ overlay.style.opacity="0"; overlay.style.pointerEvents="none"; }
